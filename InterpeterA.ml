...
type exp = Eint of int | Ebool of bool | EString of string | Den of ide | Prod of exp * exp | Sum of exp * exp | Diff of exp * exp |
	...
	Ifthenelse of exp * exp * exp | Let of ide * exp * exp | Fun of ide * exp | AccFun of ide * ide * exp | FunCall of exp * exp |
	AccFunCall of exp * exp * exp | Letrec of ide * exp * exp | Dictionary of dict | Insert of ide * exp * exp | Delete of exp * ide |
...
(*tipi esprimibili*)
type evT = Int of int | Bool of bool | String of string | Unbound | FunVal of evFun | AccFunVal of evAccFun | RecFunVal of ide * evFun | DictionaryVal of (ide * evT) list | KeyListVal of ide list
...
let rec eval (e : exp) (r : evT env) : evT = match e with
	...
	Fold(f, d) ->
				(match d with
					Dictionary(dic) -> eval (accDic f dic r) r|
					_ -> failwith("Not a dictionary")) |
	Filter(kl, d) -> 
				(match d,kl with
	...
	AccFunCall(f, eArg, eAcc) ->
		let fClosure = (eval f r) in
			(match fClosure with
				AccFunVal(arg, acc, fBody, fDecEnv) -> 
					let r1 = bind fDecEnv acc (eval eAcc r) in
						eval fBody (bind r1 arg (eval eArg r)) |
	...
	and accDic (f: exp) (dc: dict) (r: evT env) : exp =
		match dc with
			Empty -> Eint(0) |
			Item(key, v, tl) -> let nextCall = (accDic f tl r) in AccFunCall(f, v, nextCall)
